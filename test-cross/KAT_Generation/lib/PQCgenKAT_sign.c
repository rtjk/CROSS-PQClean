...
int main() {
    ...
    int                 count;
    int                 done;
    unsigned char       pk[PQCLEAN_CROSS_CLEAN_CRYPTO_PUBLICKEYBYTES], sk[PQCLEAN_CROSS_CLEAN_CRYPTO_SECRETKEYBYTES];
    int                 ret_val;

    // Create the REQUEST file
    sprintf(fn_req, "PQCsignKAT_%ld_%ld.req", PQCLEAN_CROSS_CLEAN_CRYPTO_PUBLICKEYBYTES,PQCLEAN_CROSS_CLEAN_CRYPTO_BYTES);
    if ( (fp_req = fopen(fn_req, "w")) == NULL ) {
        printf("Couldn't open <%s> for write\n", fn_req);
        return KAT_FILE_OPEN_ERROR;
    }
    sprintf(fn_rsp, "PQCsignKAT_%ld_%ld.rsp", PQCLEAN_CROSS_CLEAN_CRYPTO_PUBLICKEYBYTES,PQCLEAN_CROSS_CLEAN_CRYPTO_BYTES);
    ...
    fprintf(fp_rsp, "# %s\n\n", PQCLEAN_CROSS_CLEAN_CRYPTO_ALGNAME);
    done = 0;
    do {
        ...
        m1 = (unsigned char *)calloc(mlen+PQCLEAN_CROSS_CLEAN_CRYPTO_BYTES, sizeof(unsigned char));
        sm = (unsigned char *)calloc(mlen+PQCLEAN_CROSS_CLEAN_CRYPTO_BYTES, sizeof(unsigned char));
        ...
        if ( (ret_val = PQCLEAN_CROSS_CLEAN_crypto_sign_keypair(pk, sk)) != 0) {
            printf("PQCLEAN_CROSS_CLEAN_crypto_sign_keypair returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        fprintBstr(fp_rsp, "pk = ", pk, PQCLEAN_CROSS_CLEAN_CRYPTO_PUBLICKEYBYTES);
        fprintBstr(fp_rsp, "sk = ", sk, PQCLEAN_CROSS_CLEAN_CRYPTO_SECRETKEYBYTES);

        if ( (ret_val = PQCLEAN_CROSS_CLEAN_crypto_sign(sm, &smlen, m, mlen, sk)) != 0) {
            printf("PQCLEAN_CROSS_CLEAN_crypto_sign returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }
        ...
        if ( (ret_val = PQCLEAN_CROSS_CLEAN_crypto_sign_open(m1, &mlen1, sm, smlen, pk)) != 0) {
            printf("PQCLEAN_CROSS_CLEAN_crypto_sign_open returned <%d>\n", ret_val);
            return KAT_CRYPTO_FAILURE;
        }

        if ( mlen != mlen1 ) {
            printf("PQCLEAN_CROSS_CLEAN_crypto_sign_open returned bad 'mlen': Got <%lu>, expected <%lu>\n", mlen1, mlen);
            return KAT_CRYPTO_FAILURE;
        }

        if ( memcmp(m, m1, mlen) ) {
            printf("PQCLEAN_CROSS_CLEAN_crypto_sign_open returned bad 'm' value\n");
            return KAT_CRYPTO_FAILURE;
        }
